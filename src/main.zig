...
fn mean(comptime T: type, arr: []const T) T 
{
    ...
    return sum / T(arr.len);
}

fn median(comptime T: type, arr: []const T) ?T {
    if (arr.len == 0) return null;

    var sorted_arr = std.mem.dupe(std.heap.page_allocator, arr) catch return null;
    defer std.heap.page_allocator.free(sorted_arr);

    // Sort the array
    std.sort.sort(T, sorted_arr[0..], std.math.order(T, .lt));
    ...
    if (arr.len % 2 == 0) {
        return (sorted_arr[mid - 1] + sorted_arr[mid]) / T(2);
    } else {
